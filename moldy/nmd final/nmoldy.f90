	...
	nsize=500; nparticle=350; temp=1.0; dt=0.001; tfinal=200.0; sigma=1.0; eps=10.0

	!_______________________________________
	
	length=sigma*real(nsize)
	open(unit=11,file="nmoldyop1.dat")
	open(unit=12,file="nmoldyop2.dat")
	open(unit=13,file="nmoldyop3.dat")
	...
	do
		...
		do i=1,nparticle
			...
			sub1=abs(rpresent(i)-rpast(i))
			sub2=abs(length-sub1)
			sub=min(sub1,sub2)
			if(sub==sub1)then
				rfuture(i)=2*rpresent(i)-rpast(i)+force(i)*dt*dt/2
			else if(rpresent(i)-rpast(i)>0.0)then
				rfuture(i)=rpast(i)-2*sub+force(i)*dt*dt/2
			else
				rfuture(i)=rpast(i)+2*sub+force(i)*dt*dt/2
			end if
